<?php
    $titulo = "Formulario de alta de usuarios.";
        ...
        /* CLIENTES */
        $sql    = "SELECT cli_id,cli_nombre FROM clientes WHERE sucursal_id is null and estado = 1;";
        $roles    = mysql_query($sql);
        
        
        /* DATOS DE USUARIO*/
        $sql            = "SELECT u.id,u.cli_id,u.usu_nombre,u.usu_login FROM usuarios_portal u
                           WHERE 
                           u.id = $usu_id";
...
?>   
  ...
  <div id="main">
   ...
   <div id="contenedor" style="height:auto;">
      ...
      <form action="edit-usuarios-portal.php" method="post" name="frmEditUsuarios">
      <table class="forms" cellpadding="5">
          ...
          <tr>
            <td>Cuenta asociada con</td>
            <td><label>
              <select name="comboPerfil" class="campos" id="comboPerfil">
                <?php while($fila = mysql_fetch_array($roles)){?>
                <option value="<?php echo $fila["cli_id"]?>" <?php if($fila_usuario["cli_id"] == $fila["cli_id"]){echo(" selected=\"selected\"");} ?>>
