<?php
    ...
    $titulo = "Formulario de modificaci&oacute;n de una Orden de Servicio.";
        ...
        $fila0 = mysql_fetch_array($resultado0);
         // valido si es sucursal matriz
           $cli_de_orden=$fila0["cli_id"];   
           $sql = "SELECT cli_id, cli_nombre,sucursal,sucursal_id 
           FROM clientes
           WHERE cli_id = $cli_de_orden";
           
           $resultadoClienteDatosConsulta = mysql_query($sql);
            $resultadoClienteDatos = mysql_fetch_array($resultadoClienteDatosConsulta);
           
           if ($resultadoClienteDatos["sucursal_id"]=='')//Si no tiene sucursal => es Casa Matriz
           {
               $ClienteParaCombo=$resultadoClienteDatos["cli_id"];
               $SucursalParaCombo="";
           }
           else
           {
             $ClienteParaCombo= $resultadoClienteDatos["sucursal_id"];
             $SucursalParaCombo=$resultadoClienteDatos["cli_id"];
           }
           echo $ClienteParaCombo," sucursal: ",$SucursalParaCombo;
         //Clientes
   ...
   <div id="contenedor" style="height:auto;">
    ...
    ?>
       ...
       <tr>
         ...
         <td><select name="cli_id" id="cli_id" class="campos" required onChange="habilitarCombo2('cli_id','suc_id')" >
    ...
    ?>
           <option value="<?php echo($fila["cli_id"]); ?>"<?php if($ClienteParaCombo==$fila["cli_id"]){echo(" selected=\"selected\"");} ?>><?php echo(utf8_encode($fila["cli_nombre"])); ?> (<?php echo(utf8_encode($fila["provincia"])); ?>/<?php echo(utf8_encode($fila["sucursal"])); ?>)</option>
           <?php
          }
    ?>
         </select></td>
       ...
       <tr>
         ...
         <td><select name="suc_id" id="suc_id" class="campos" required <?php if($SucursalParaCombo==""){echo ("disabled");}?>>
           <option value='todasLasSucursales'>Todas las Sucursales</option>
                    
           <?php
          while($fila = mysql_fetch_array($resultadoSucursales)){
    ?>
           <option value="<?php echo($fila["cli_id"]); ?>"<?php if($SucursalParaCombo==$fila["cli_id"]){echo(" selected=\"selected\"");} ?>><?php echo(utf8_encode($fila["cli_nombre"])); ?> (<?php echo(utf8_encode($fila["sucursal"])); ?>)</option>
           <?php
          }
    ?>          
           </select></td>
